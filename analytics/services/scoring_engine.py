# analytics/services/scoring_engine.py
...
def consistency_score(log_entries: list[str]) -> float:
...
def effort_score(log_entries: list[str]) -> float:
...
def compute_intern_score(log_entries: list[str], ai_report: dict) -> int:
